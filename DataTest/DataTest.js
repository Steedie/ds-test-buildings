...
function encodeCall(functionSignature, parameters) {
    // Create an Interface object for the ABI encoding
    const iface = new ethers.utils.Interface([functionSignature]);

    // Encode the function call with parameters
    const data = iface.encodeFunctionData(iface.getFunction(functionSignature), parameters);
...
export default async function update(state) {
    ...
    const incrementCount = () => {
        ...
        const payload = encodeCall("hello(bytes24)", [
            ethers.utils.formatBytesString(`Hello, the count is ${count}`, 24)
        ]);
