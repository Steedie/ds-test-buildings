...
abstract contract DataTest is BuildingKind {
    function hello(bytes24 helloText) virtual public;
    function use(Game ds, bytes24 buildingInstance, bytes24 unit, bytes calldata payload) public {
        State s = GetState(ds);
        if (bytes4(payload) == this.hello.selector) {
            (bytes24 helloText) =
                abi.decode(payload[4:], (bytes24));
                _receiveHelloText(s, buildingInstance, helloText);
        }
    }

    function _receiveHelloText(
        State s,
        bytes24 buildingInstance,
        bytes24 helloText
        ) private {

            bytes24 buildingKind = s.getBuildingKind(buildingInstance);
            s.annotate(
                buildingKind,
                "description",
                "test test test"
        ...
        //ds.getDispatcher().dispatch(abi.encodeCall(TestActions.hello, (helloText)));
    }
